import { RawMagentoProduct } from "../../../interfaces/magento/product";
...
export const syncProducts = async (verbose?: boolean) => {
  ...
  if (firstRequest.total) {
    ...
    for (let i = 0; i < Math.ceil(total / 50); i++) {
      ...
      let pricesRes = await sendGetRequest(
        `/product/listByIds?${res.products
          .map((product: RawMagentoProduct) => product.id)
          .join(",")}`
      );
      if (res) await saveProducts(res.products, pricesRes.products);
      if (verbose)
        console.log(
